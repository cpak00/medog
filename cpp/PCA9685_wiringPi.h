...
int pca9685_init(unsigned char addr);	// addr��?7位的i2c从机地址，返回的是linux标准的设备描述符，调用它的地方视作pca9685的设备描述符
					     //因为可以多个pca9685级联，通过设备描述符区别它��?
					     //此驱动仅作为驱动舵机使用，周期固定死��?20ms，不允许外部设置
void pca9685_setmk(int fd, int num, int mk); //设置指定通道的脉宽。fd是在pca9685_init时获得的设备描述符，num是通道号（��?0开始），mk是脉宽单位是us。周期已经固定为20ms��?
...
void pca9685_setmk(int fd, int num, int mk) //设置指定通道的脉宽。fd是在pca9685_init时获得的设备描述符，num是通道号（��?0开始），mk是脉宽单位是us。周期已经固定为20ms��?
{
    unsigned int ON, OFF;
    ON = 0;							     //每次周期一开始就输出高电��?
    OFF = (unsigned int)((((double)mk) / 20000 * 4096) * 1.0067114); //最后的1.0067114是校准用��?
								     //    printf("off = 0x%x", OFF);
